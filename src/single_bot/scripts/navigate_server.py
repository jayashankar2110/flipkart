...
class NavigationServer():
    ...
    def _goal_cb(self):
        ...
        #uncomment for taking real feedback frm camera
        # global c_state
        # if c_state is None:
        #     c_state = c_state0
        ...
        while T >= time and lastIndex > target_ind and not rospy.is_shutdown():
            ...
            state.update(ai, di)  # Control vehicle via commu node
            self._com_pub.publish(vr = np.float32(ai),vl = np.float32(di),ifUnload = False,isLoaded=True)
            #rospy.loginfo(ai)
            time += dt  #use ros time
            # update time elapsed
